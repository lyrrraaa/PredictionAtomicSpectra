...
# ----------------------------
# Profesional Photon Energy vs Wavelength (Dengan Latar Spektrum)
# ----------------------------
...
# Tambahkan latar belakang spektrum warna (violet → merah)
for i in range(len(wavelengths)-1):
    r, g, b = wavelength_to_rgb(wavelengths[i])
    fig2.add_shape(
        type="rect",
        x0=wavelengths[i], x1=wavelengths[i+1],
        y0=0, y1=max(energies)*1.05,
        fillcolor=f"rgba({int(r*255)},{int(g*255)},{int(b*255)},0.25)",  # transparansi lembut
        line=dict(width=0)
    )
...
# Titik hasil eksperimen
for wl, E in zip(results["λ (nm)"], results["E (eV)"]):
    if not np.isnan(wl):
        fig2.add_trace(go.Scatter(
            x=[wl], y=[E],
            mode="markers",
            marker=dict(size=9, color="crimson", line=dict(width=1.5, color="white")),
            showlegend=False
        ))

# Layout elegan
fig2.update_layout(
    xaxis_title="<i>Wavelength (nm)</i>",
    yaxis_title="<i>Photon Energy (eV)</i>",
    plot_bgcolor="white",
    paper_bgcolor="white",
    font=dict(color="#111", family="Poppins", size=14),
    xaxis=dict(gridcolor="rgba(0,0,0,0.1)", zeroline=False, range=[380,780]),
    yaxis=dict(gridcolor="rgba(0,0,0,0.1)", zeroline=False),
    height=420,
    margin=dict(l=60, r=40, t=60, b=60),
    title=dict(
        text="Photon Energy vs Wavelength (Emission Spectrum Background)",
        x=0.5,
        font=dict(size=20, color="#111", family="Poppins", weight="bold")
    )
)

# Tambah anotasi penjelas
fig2.add_annotation(
    xref="paper", yref="paper", x=0.98, y=0.95,
    text="Shorter λ → Higher E",
    showarrow=False,
